...
// 数据库文件路径（可通过 KB_DB_PATH 环境变量覆盖，便于测试与自定义部署）
export const dbPath = process.env.KB_DB_PATH || path.join(__dirname, '..', 'data', 'chat.db');
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS messages (
    ...
    created_at TEXT NOT NULL,
    tool_calls TEXT,
    kb_sources TEXT,            -- 知识库检索来源（JSON 数组），供刷新会话后恢复引用标签
...
// 数据库迁移：添加 kb_sources 列（如果不存在）
try {
  const msgInfo = db.prepare("PRAGMA table_info(messages)").all() as Array<{ name: string }>;
  if (!msgInfo.some(col => col.name === 'kb_sources')) {
    db.exec("ALTER TABLE messages ADD COLUMN kb_sources TEXT");
    console.log("[DB] Added kb_sources column to messages table");
  }
} catch (e) {
  // 忽略错误（列可能已存在）
}
...
export function createMessage(message: DbMessage): DbMessage {
  const stmt = db.prepare(`
    INSERT INTO messages (id, session_id, role, content, model, created_at, tool_calls, kb_sources)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  stmt.run(
    message.id,
...
// 跨会话搜索历史消息（按内容模糊匹配）
export function searchMessages(query: string, limit = 50): Array<DbMessage & { session_title: string }> {
  const like = `%${query}%`;
  return db.prepare(`
    ...
    LIMIT ?
  `).all(like, limit) as Array<DbMessage & { session_title: string }>;
}
...
export function createMessages(messages: DbMessage[]): void {
  ...
  const insertMany = db.transaction((msgs: DbMessage[]) => {
    for (const msg of msgs) {
      stmt.run(msg.id, msg.session_id, msg.role, msg.content, msg.model, msg.created_at, msg.tool_calls, msg.kb_sources ?? null);
    }
  });
