...
export function useChat(options: UseChatOptions) {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.type === 'init') {
                  ...
                  // OpenAI 兼容等模式下上游返回的错误（缺少 Key / 网络异常 / API 报错）
                  const errText = data.content || '发生未知错误';
                  setSessions(prev => prev.map(s => {
                    if (s.id === realSessionId) {
                      return {
                        ...s,
  ...
  }, [currentSession, currentSessionId, selectedModel, useKb, getAgent, updateSessionModel, setCurrentSessionId, setSessions, isLoading]);
